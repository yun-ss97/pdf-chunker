...
with st.sidebar:
    st.header("설정")
    max_tokens = st.number_input("청크당 최대 토큰 수", min_value=500, max_value=20000, value=4000, step=500)
    overlap = st.number_input("청크 간 겹치는 문장 수", min_value=0, max_value=20, value=3, step=1)
    save_dir = st.text_input(
        "저장 경로 (서버/로컬 기준)",
        value="./",
        help="브라우저 사용자의 PC 경로가 아니라, 앱이 실행되는 환경 기준 경로입니다."
    )
...
if run:
    ...
    with st.spinner("PDF 처리 중입니다..."):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            output_dir = Path(save_dir).expanduser().resolve() / f"chunks_{pdf_name}"
            ...
            filenames, readme_path = save_chunks(chunks, str(output_dir), pdf_name)
            update_progress(
                stage="done",
                current=100,
                total=100,
    ...
    c1.metric("총 페이지", len(pages))
    c2.metric("빈 페이지", empty_pages)
    c3.metric("총 토큰", f"{total_tokens:,}")
    c4.metric("총 청크", len(chunks))

    st.info(f"저장 경로: `{output_dir}`")

    # with st.expander("청크 미리보기", expanded=True):
    #     preview_count = min(3, len(chunks))
    #     for i in range(preview_count):
    #         st.markdown(f"**chunk_{i+1}**")
    #         st.code(chunks[i]["text"][:2000], language="text")
